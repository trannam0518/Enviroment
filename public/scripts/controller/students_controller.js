app.controller('students_ctl', ['$scope', '$http', '$window', '$compile', '$timeout', function ($scope, $http, $window, $compile, $timeout) {
	...
	var loadtable = function () {
		...
		var t = jQuery("#data_table").DataTable({
			...
			//"processing": true,
			"deferRender": true,
			"aaData": $scope.students_list,
			"rowId": "code_stu",
			"aoColumns": [
				{ "data": "No", "sWidth": "5%" },
				{ "data": "id_stu", "sClass": "text" },
...

				{
					"data": null, mRender: function (data, type, row) {
								...
								str = (month > 10 ? month : "0" + month) + "/" + (date > 10 ? date : "0" + date) + "/" + birthday.getFullYear();
								return str;
					}		
				},				
				{ "data": "address", "sClass": "text" },			
